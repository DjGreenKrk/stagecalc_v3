...
export default function ClientsPage() {
  ...
  const clientsCollection = useMemoFirebase(
    () => user ? query(collection(firestore, 'clients'), where('ownerUserId', '==', user.uid)) : null,
    [firestore, user]
  );
  const { data: allClients, isLoading } = useCollection<Client>(clientsCollection);
  ...
  const handleSave = (client: Omit<Client, 'id' | 'ownerUserId'>) => {
    if (!user) return;
    ...
    if (selectedClient?.id) {
      const clientRef = doc(firestore, 'clients', selectedClient.id);
      updateDocumentNonBlocking(clientRef, dataToSave);
    } else {
      addDocumentNonBlocking(collection(firestore, 'clients'), dataToSave);
    }
    setIsFormOpen(false);
  };
              ...
              {paginatedClients.length === 0 && !isLoading && (
                 <TableRow>
                  <TableCell colSpan={5} className="text-center py-10 h-60">
                    ...
                    <p className="text-muted-foreground">{searchQuery ? t('clients.try_different_query') : t('clients.add_first_client')}</p>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('clients.delete_warning_description', { clientName: selectedClient?.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

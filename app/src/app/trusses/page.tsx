...
import { PlusCircle, Search, MoreHorizontal, Edit, Trash2, Weight } from 'lucide-react';
  ...
  return (
    <AppShell>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold font-headline">Kalkulacje nośności kratownic</h1>
      ...
      <Card>
        <CardContent className="pt-6">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Nazwa kalkulacji</TableHead>
              ...
              {filteredCalculations.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className="text-center h-60">
                    <Weight className="mx-auto h-12 w-12 text-muted-foreground/50" />
                    <p className="mt-4 text-lg">Brak kalkulacji</p>
                    <p className="text-muted-foreground">Utwórz swoją pierwszą kalkulację, aby zacząć planowanie nośności.</p>
                ...
                filteredCalculations.map(calc => (
                  <TableRow key={calc.id}>
                    <TableCell className="font-medium">
                      <Link href={`/trusses/${calc.id}`} className="text-primary hover:underline">
                        {calc.name}
                      </Link>
                    </TableCell>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tej akcji nie można cofnąć. Spowoduje to trwałe usunięcie kalkulacji "{selectedCalculation?.name}".
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

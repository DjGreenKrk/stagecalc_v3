...
export default function CategoryPage() {
  ...
  const { data: rawDevices, isLoading: areDevicesLoading } = useCollection<Device>(devicesQuery);
  ...
  useEffect(() => {
    // Only check for redirect if user loading is finished and user exists
  ...
  const toggleFavorite = (deviceId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!userFavoritesDocRef || !user) return;
    ...
    const dataToSave = { userId: user.uid, deviceIds: Array.from(newFavorites) };
    setDocumentNonBlocking(userFavoritesDocRef, dataToSave, { merge: true });
  };
  ...
  return (
    <AppShell>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold font-headline">{t(`categories.${categorySlug}`, categoryInfo.name)}</h1>
      ...
      <div className="mb-4 flex items-center gap-2 flex-wrap">
        {categoryInfo.subcategories && categoryInfo.subcategories.length > 0 && (
          <>
            ...
            {categoryInfo.subcategories.map(sub => (<Badge key={sub.key} variant={activeSubcategory === sub.key ? 'default' : 'secondary'} className="cursor-pointer" onClick={() => setActiveSubcategory(sub.key)}>{t(`devices.subcategories.${sub.key}`, sub.label)}</Badge>))}
          </>
        )}
        {categorySlug === 'sound' && (
           <Popover>
            ...
            <TableBody>
              ...
              {!areDevicesLoading && paginatedDevices.length > 0 && paginatedDevices.map((device) => {
                ...
                <TableRow key={device.id}>
                  ...
                  <TableCell className="font-medium break-words"><button onClick={() => handleViewDetails(device)} className="text-primary hover:underline">{device.name}</button>{device.subcategory && <Badge variant="outline" className="ml-2">{t(`devices.subcategories.${device.subcategory}`, device.subcategory)}</Badge>}</TableCell>

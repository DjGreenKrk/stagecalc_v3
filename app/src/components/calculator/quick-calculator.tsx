...
import { collection, doc, onSnapshot, addDoc, serverTimestamp, getDoc, getDocs } from 'firebase/firestore';
import { useRouter, useParams } from 'next/navigation';
import type { Device, Calculation, CalculationGroup, CalculationItem, PowerConnector, Location, EventDevice, Client, UserFavoriteDevices, DeviceCategoryName, DeviceCategory, PowerConnectorGroup } from '@/lib/definitions';
...
export function QuickCalculator({ initialData }: { initialData?: Calculation }) {
  ...
  const allDeviceQueries = useMemoFirebase(() => {
    if (!user || !firestore) return [];
    return deviceCategories.map(c => collection(firestore, c.collectionName));
  }, [user, firestore]);

  useEffect(() => {
    if (!allDeviceQueries.length) {
      if (!firestore) {
         setIsLoading(true);
      } else {
         setIsLoading(false);
    ...
    const unsubscribes = allDeviceQueries.map((q, index) => 
      onSnapshot(q, (snapshot) => {
        const categoryName = deviceCategories[index].name;
        const devicesFromCollection = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id, category: categoryName } as Device));
        
        setDeviceCatalog(prev => {
          const otherDevices = prev.filter(p => p.category !== categoryName);
          return [...otherDevices, ...devicesFromCollection];
        });
        
        loadedCount++;
        if (loadedCount === allDeviceQueries.length) {
          setIsLoading(false);
        }
      }, (error) => {
        console.error(`Error loading ${deviceCategories[index].collectionName}:`, error);
        loadedCount++;
        if (loadedCount === allDeviceQueries.length) {
          setIsLoading(false);
        }
  ...
  }, [allDeviceQueries, firestore]);


  const locationsQuery = useMemoFirebase(() => user ? collection(firestore, 'locations') : null, [firestore, user]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (params.id && params.id !== 'new') {
        const docRef = doc(firestore, 'calculations', params.id as string);
        setDocumentNonBlocking(docRef, sanitizedData, { merge: true });
        toast({ title: "Kalkulacja zaktualizowana!" });
      } else {
        const newDocRef = await addDoc(collection(firestore, 'calculations'), sanitizedData);
        toast({ title: "Kalkulacja zapisana!" });
        if (newDocRef.id) {
          router.push(`/calculators/${newDocRef.id}`);
        }
      }
    } catch (error: any) {
    ...
    return connectorGroups.map(group => {
      return {
        ...
        connectors: group.connectors.map(connector => {
          ...
          return {
            ...connector,
            name: getConnectorDisplayName(connector, activeConnectors),
            isOverloaded,
            totalLoad,
            totalCapacity,
          };
                              ...
                              <Popover open={openMultiSelects[group.tempId]} onOpenChange={(open) => toggleMultiSelect(group.tempId, open)}>
                                <PopoverTrigger asChild>
                                  <Button variant="outline" role="combobox" aria-expanded={openMultiSelects[group.tempId]} className="w-[150px] sm:w-[200px] justify-between h-9">
                                    <span className="truncate">{group.assignedConnectorIds?.length > 0 ? `${group.assignedConnectorIds.length} wybrano` : 'Przypisz przyłącza...'}</span>
                                  </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                  <Command>
                                        ...
                                        <CommandGroup key={connectorGroup.id} heading={connectorGroup.name}>
                                          {connectorGroup.connectors
                                            ...
                                            >
                                              ...
                                              {getConnectorDisplayName(c, activeConnectors)}
                                            </CommandItem>
                                          ))}
                                        </CommandGroup>
                                      ))}
                                    ...
                                    <SelectContent>
                                        ...
                                        {availablePowerSources.map(({ parentGroup, connector }) => (
                                          <SelectItem key={connector.instanceId} value={`${parentGroup.id}|${connector.instanceId}`}>
                                            {`${parentGroup.name} - ${getConnectorDisplayName(connector, activeConnectors)}`}
                                          </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                          ...
                          <div className="grid gap-4">
                            ...
                            {group.connectors.map((connector) => (
                              <div key={connector.instanceId} className={cn("p-4 border rounded-lg relative bg-background", connector.isOverloaded && "border-destructive bg-destructive/10")}>
                                ...
                                <p className="font-medium pr-6">{getConnectorDisplayName(connector, activeConnectors)}</p>
                                <div>

...
export function TrussCalculator({ calculation }: TrussCalculatorProps) {
  ...
  useEffect(() => {
    ...
    Promise.all(allDeviceQueries.map(q => new Promise(resolve => onSnapshot(q, resolve)))).then(() => {
        setIsLoadingCatalog(false);
    });
  ...
  const riggingDevicesQuery = useMemoFirebase(() => {
    if (!user) return null;
    return collection(firestore, 'rigging_devices');
  }, [firestore, user]);
  const { data: riggingDevices } = useCollection<Device>(riggingDevicesQuery);
  ...
  const handleSaveCalculation = async (updatedTrusses: Truss[], updatedGroups: CalculationGroup[]) => {
    ...
    try {
      const docRef = doc(firestore, 'calculations', calculation.id);

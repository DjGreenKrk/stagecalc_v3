...
export function LocationDetailsDialog({
  ...
  const { totalPowerCapacity, totalPowerCapacityNumber } = useMemo(() => {
    if (!allConnectors || allConnectors.length === 0) return { totalPowerCapacity: '0.0', totalPowerCapacityNumber: 0 };
    ...
    const num = totalPower / 1000;
    return { totalPowerCapacity: num.toFixed(1), totalPowerCapacityNumber: num };
  }, [allConnectors]);
